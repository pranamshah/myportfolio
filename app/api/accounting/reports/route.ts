...
export async function GET(req: NextRequest) {
  ...
  for (const v of vouchers) {
    for (const e of v.entries) {
      if (!balances[e.ledgerId]) balances[e.ledgerId] = { dr: 0, cr: 0 };
      if (e.type === "DR") balances[e.ledgerId].dr += e.amount;
      else balances[e.ledgerId].cr += e.amount;
    }
  }

  if (report === "daybook") {
    const dayVouchers = await prisma.voucher.findMany({
      where: { date: { gte: from, lte: to } },
  ...
  if (report === "ledger") {
    ...
    const matchingVouchers = await prisma.voucher.findMany({
      where: { date: { gte: from, lte: to }, entries: { some: { ledgerId } } },
      include: { entries: true },
      orderBy: { date: "asc" },
    });
