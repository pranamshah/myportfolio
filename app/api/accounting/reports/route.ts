...
export async function GET(req: NextRequest) {
  ...
  const from = searchParams.get("from") ? new Date(searchParams.get("from")!) : new Date(new Date().getFullYear(), 3, 1); // Apr 1
  ...
  // Build ledger balances from voucher entries
  ...
  if (report === "trial") {
    const rows = ledgers.map(l => {
      ...
      const netDR = b.dr + openDR;
      const netCR = b.cr + openCR;
