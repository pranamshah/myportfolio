...
export async function POST(req: NextRequest) {
  ...
  const doc = new ShipDoc({
    shipment, uploadedBy: session.user.id, name, originalName: file.name,
    fileType: file.type, fileSize: file.size, filePath: `/uploads/${shipment}/${filename}`,
    category, description, isVisibleToClient,
  });
  await doc.save();
